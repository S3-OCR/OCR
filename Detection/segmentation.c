...
SDL_Surface* columns(SDL_Surface* img, int b_line, int e_line)
{
	...
	int white = 0;// trace last line of the caracter
	...
	//in wich there is another loop for all the columns
  	...
  	for(x = 0; x < w; x++)// through the columns
  	{
		for(y = b_line; y < e_line; y++)// through the lines
		{
			...
			// had encountered a character but now it's its end 
			if (b == 255 && end_c)
			{
				end_c = 0;// the end of caracter is here
				white = 1;// must trace the last line
				break;
			}
		}
		...
		if (!end_c && white)// color the next column of the caracter
		{
			white = 0;// last line was traced, so make back the statement
			for (y = b_line; y < e_line; y++)
			{
				if (x + 1 < w)
                                {
